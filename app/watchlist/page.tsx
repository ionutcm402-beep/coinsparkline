...
export default async function WatchlistPage() {
  ...
  const items: WatchlistItem[] = [
    ...tracked.map(c => {
      ...
      return { id:c.id, name:c.name, symbol:c.symbol.toUpperCase(), image:c.logoUrl||"", price:c.price, change24h:c.change24hPct, rank:c.marketCapRank??null, regime:TIER_CONFIG[tier].label, confidence:c.confidencePct, sparkScore:getSparkScore(c).score };
    }),
  ...
  return <div className="min-h-screen bg-[#fbfcff] text-slate-950"><Header/><main className="mx-auto max-w-[1100px] px-4 pb-16 pt-8 sm:px-6 sm:pt-10"><div className="mx-auto max-w-2xl text-center"><p className="text-[9px] font-bold uppercase tracking-[0.18em] text-violet-500">Your market</p><h1 className="mt-2 text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">Watchlist</h1><p className="mx-auto mt-2 max-w-lg text-xs leading-5 text-slate-500">Keep the coins you care about in one place. Signed-in users sync their watchlist across devices; guests can still save coins on this device.</p></div><WatchlistClient items={items}/></main></div>;
}
