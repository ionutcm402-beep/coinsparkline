...
export default function Header() {
  ...
  const navigation = [{href:"/",label:"Market"},{href:"/watchlist",label:"Watchlist"},{href:"/compare",label:"Compare"},{href:"/alerts",label:"Alerts"},{href:"/about",label:"About"}];
  return <header className="csl-site-header"><div className="csl-header-inner" style={{minHeight:"3.55rem"}}>
    ...
    <div className="flex items-center gap-2 sm:gap-2.5"><nav className="csl-primary-nav" aria-label="Primary navigation" style={{minHeight:"2.05rem",padding:".12rem"}}>{navigation.map(item=>{const active=pathname===item.href||item.href!=="/"&&pathname.startsWith(item.href+"/");return <Link key={item.href} href={item.href} className={`csl-nav-link${active?" csl-nav-link--active":""}`} style={{minHeight:"1.65rem",padding:".34rem .52rem",fontSize:".68rem"}} aria-current={active?"page":undefined}>{item.label}</Link>})}</nav>
