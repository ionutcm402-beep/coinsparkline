...
function Wordmark() {
  return (
    <span className="csl-wordmark" aria-hidden="true" style={{ display: "inline-flex", alignItems: "baseline", gap: "0", fontSize: ".94rem", letterSpacing: "-0.045em", lineHeight: 1, whiteSpace: "nowrap" }}>
...
export default function Header() {
  ...
  return (
    <header className="csl-site-header">
      <div className="csl-header-inner" style={{ minHeight: "3.55rem" }}>
        <Link href="/" className="csl-brand" aria-label="CoinSparkLine home" style={{ gap: "0.32rem" }}>
          <LogoMark />
          <span className="hidden min-[420px]:inline"><Wordmark /></span>
        </Link>

        <div className="flex items-center gap-2.5 sm:gap-3">
          <nav className="csl-primary-nav" aria-label="Primary navigation" style={{ minHeight: "2.15rem", padding: ".15rem" }}>
            {navigation.map((item) => {
              const isActive = pathname === item.href;
              return (
                <Link key={item.href} href={item.href} className={`csl-nav-link${isActive ? " csl-nav-link--active" : ""}`} style={{ minHeight: "1.75rem", padding: ".38rem .62rem", fontSize: ".72rem" }} aria-current={isActive ? "page" : undefined}>
                  {item.label}
                </Link>
              );
            })}
          ...
          <div className="hidden items-center rounded-full border border-slate-200/80 bg-white/85 p-0.5 shadow-[0_4px_12px_rgba(20,35,75,0.035)] sm:flex" aria-label="Display currency">
            {(["USD", "GBP", "EUR"] as DisplayCurrency[]).map((code) => (
              <button
                key={code}
                type="button"
                onClick={() => setCurrency(code)}
                className={`rounded-full px-2 py-0.5 text-[9px] font-semibold transition-all ${currency === code ? "bg-slate-950 text-white shadow-sm" : "text-slate-500 hover:text-slate-900"}`}
          ...
          <select
            ...
            className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[10px] font-semibold text-slate-700 sm:hidden"
