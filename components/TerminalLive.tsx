...
import TerminalMacroPulse from "@/components/TerminalMacroPulse";
...
export default function TerminalLive({coins,updatedLabel}:{coins:Coin[];updatedLabel?:string}){
 ...
 return <main className="terminal-root terminal-v1-next">
  <header className="terminal-topbar"><Link href="/" className="terminal-brand">Coin<span>SparkLine</span></Link><nav aria-label="Terminal navigation"><a href="#live" data-active="true">LIVE</a><Link href="/radar">RADAR</Link><Link href="/signals">MY SIGNALS</Link></nav><div className="terminal-top-actions"><span className="terminal-snapshot">{updatedLabel||"Latest market snapshot"}</span><Link href="/methodology">Methodology</Link></div></header>
  <nav className="terminal-market-nav" aria-label="Market classes"><a href="#crypto" data-active="true"><b>CRYPTO</b><span>Spark intelligence</span></a><a href="#macro"><b>GOLD + FX</b><span>Live market</span></a><a href="#nft"><b>NFT</b><span>OpenSea activity</span></a><span className="terminal-market-nav-locked"><b>STOCKS</b><span>Licensed feed slot</span></span></nav>
  <section className="terminal-market-strip" id="crypto" aria-label="Current crypto market state"><div><span>MARKET STATE</span><strong>{marketLabel}</strong></div><div><span>SPARK PRESSURE</span><strong>{avgScore}<small>/100</small></strong></div><div><span>HIGH-INTENSITY SIGNALS</span><strong>{unusual}</strong></div><div><span>VOLATILE</span><strong>{volatile}</strong></div><div><span>BUILDING / AWAKENING</span><strong>{building+awakening}</strong></div></section>
  ...
  <div className="terminal-ticker" aria-label="Crypto market ticker"><div>{ranked.slice(0,14).map(c=><button key={c.id} onClick={()=>setSelectedId(c.id)}><b>{c.symbol.toUpperCase()}</b><span><CurrencyAmount usd={c.price}/></span><em data-direction={c.change24hPct>=0?"up":"down"}>{pct(c.change24hPct)}</em><i>SPK {getSparkScore(c).score}</i></button>)}</div></div>
  <div className="terminal-workspace" id="live">
  ...
  <div className="terminal-cross-market"><div id="macro"><TerminalMacroPulse/></div><div id="nft"><TerminalNftPulse/></div></div>
 </main>
}
