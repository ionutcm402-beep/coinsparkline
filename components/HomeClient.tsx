...
interface HomeClientProps { coins: Coin[]; marketCoins?: MarketBreadthCoin[]; categories: string[]; updatedLabel?: string; }
...
export default function HomeClient({coins,marketCoins=[],categories,updatedLabel}:HomeClientProps){
 ...
 return <div className="mx-auto max-w-[1320px] space-y-7 px-4 sm:px-5"><MarketRegimeSummary coins={coins} updatedLabel={updatedLabel}/>
  ...
  <section className="rounded-[22px] border border-slate-200/70 bg-white/76 px-3 py-4 shadow-[0_10px_34px_rgba(20,35,75,0.045)] backdrop-blur sm:px-4">
    ...
    <p className="mt-3 text-center text-[9px] leading-4 text-slate-400">Radar reflects current regime, confidence and streak freshness. Exact day-over-day changes will be added in the next snapshot-comparison stage.</p>
  </section>
