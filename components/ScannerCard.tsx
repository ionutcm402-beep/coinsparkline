...
export default function ScannerCard({ coin }: { coin: Coin }) {
  const [open,setOpen]=useState(false); const tier=getSignalTier(coin); const config=TIER_CONFIG[tier]; const positive=coin.change24hPct>=0; const spark=getSparkScore(coin);
  return <article className="group relative overflow-hidden rounded-[13px] border border-slate-200/70 bg-white/88 text-center shadow-[0_4px_14px_rgba(20,35,75,0.025)] transition-all hover:-translate-y-0.5 hover:border-slate-300 hover:bg-white hover:shadow-[0_9px_22px_rgba(20,35,75,0.065)]">
    ...
    <div className="mb-1.5 flex items-center justify-center gap-1 px-1.5"><button type="button" onClick={()=>setOpen(v=>!v)} aria-expanded={open} className="rounded-full border border-slate-200 bg-white px-2 py-1 text-[8px] font-semibold text-slate-400 hover:text-slate-700">Why {open?"−":"+"}</button><AlertRuleButton coinId={coin.id} compact/></div>
    {open&&<div className="border-t border-slate-100 bg-slate-50/85 px-2.5 py-2 text-left"><div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[8px]"><span className="text-slate-400">Confidence</span><strong className="text-right text-slate-700">{spark.confidence.toFixed(0)}</strong><span className="text-slate-400">Freshness</span><strong className="text-right text-slate-700">{spark.freshness.toFixed(0)}</strong><span className="text-slate-400">Flip pressure</span><strong className="text-right text-slate-700">{spark.transitionPressure.toFixed(0)}</strong><span className="text-slate-400">24h activity</span><strong className="text-right text-slate-700">{spark.marketMove.toFixed(0)}</strong></div><p className="mt-1.5 text-[7px] leading-3 text-slate-400">Behavioural activity only — not expected return.</p></div>}
  </article>;
}
