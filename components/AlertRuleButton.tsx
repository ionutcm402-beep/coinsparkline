...
export default function AlertRuleButton({coinId}:{coinId:string}){
 ...
 return <button type="button" onClick={toggle} className={`rounded-full border px-3 py-1.5 text-[10px] font-semibold transition ${armed?"border-violet-200 bg-violet-50 text-violet-700":"border-slate-200 bg-white text-slate-500 hover:text-slate-900"}`}>{armed?"Alert armed ✓":"Alert on regime change"}</button>;
}
